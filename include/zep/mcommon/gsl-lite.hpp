...
public:
    ...
    gsl_api final_action(final_action &&other) gsl_noexcept
        : action_(std::move(other.action_)), invoke_(other.invoke_) {
        other.invoke_ = false;
    }
...
template<
    class Container, class ElementType, class = typename std::enable_if<
...
public:
    ...
    gsl_api gsl_constexpr14 span() gsl_noexcept
        : first_(gsl_nullptr), last_(gsl_nullptr) {
        Expects(size() == 0);
    }
    ...
    gsl_api gsl_constexpr span(U (&arr)[N]) gsl_noexcept
        : first_(gsl_ADDRESSOF(arr[0])), last_(gsl_ADDRESSOF(arr[0]) + N) {}
#else
    template< size_t N
# if gsl_HAVE( DEFAULT_FUNCTION_TEMPLATE_ARG )
