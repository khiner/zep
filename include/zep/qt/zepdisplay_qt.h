...
public:
    ...
    void DrawChars(ZepFont &font, const NVec2f &pos, const NVec4f &col, const uint8_t *text_begin, const uint8_t *text_end) const override {
        ...
        m_pPainter->drawText(p0.x(), p0.y() - qtFont.Descent() + GetPixelScale().y * 1, m_pPainter->viewport().width() - p0.x(), m_pPainter->viewport().height() - p0.y(), Qt::TextLongestVariant,
    ...
    virtual ZepFont &GetFont(ZepTextType type) override {
        if (m_fonts[(int) type] == nullptr) {
            ...
            switch (type) {
                case ZepTextType::Heading1:height *= 1.75f;
                    break;
                case ZepTextType::Heading2:height *= 1.5f;
                    break;
                case ZepTextType::Heading3:height *= 1.25f;
                    break;
            }
